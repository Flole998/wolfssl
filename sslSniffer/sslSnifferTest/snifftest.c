...
#if defined(HAVE_PTHREAD) && !defined(NO_THREADED_SNIFFTEST)
    #include <pthread.h>
    #ifndef THREADED_SNIFFTEST
        #define THREADED_SNIFFTEST
    #endif
#endif
...
#ifndef THREADED_SNIFFTEST_WORKERS
    /* Change to increase number of threads */
    #define THREADED_SNIFFTEST_WORKERS 4
#endif

static const int workerThreadCount = THREADED_SNIFFTEST_WORKERS;
...
static void* snifferWorker(void* arg)
{
    ...
    while (1) {
        ...
        /* break if worker has shutdown or if worker is invalid (unutilized) */
        if ((worker->shutdown && packet == NULL) || worker->head->valid == 0) {
            break;
        }

        /* continue if current head has already been processed */
        if (worker->lastProcessed == worker->head->packetNumber) {
            continue;
        }

        /* Decode Packet, ret value will indicate whether a
         * bad packet was encountered */
        worker->hadBadPacket = DecodePacket(packet, length, packetNumber, err);
...
int main(int argc, char** argv)
{
    ...
    for (i=0; i<workerThreadCount; i++) {
        ssl_Init_SnifferWorker(&workers[i], port, server, keyFilesSrc, passwd);
